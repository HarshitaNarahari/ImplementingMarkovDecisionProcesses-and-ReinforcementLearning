import numpy as np
from tqdm import tqdm
import gymnasium as gym
import matrix_mdp
import sys
import matplotlib.pyplot as plt

nS = 16
nA = 4

slip_prob = 0.1

actions = ['up', 'down', 'left', 'right']

p_0 = np.array([0 for _ in range(nS)])
p_0[12] = 1

P = np.zeros((nS,nS,nA), dtype=float)

def valid_neighbors(i,j):
	neighbors = {}
	if i>0:
		neighbors[0]=(i-1,j)
	if i<3:
		neighbors[1]=(i+1,j)
	if j>0:
		neighbors[2]=(i,j-1)
	if j<3:
		neighbors[3]=(i,j+1)
	return neighbors

for i in range(4):
	for j in range(4):
		if i==0 and j==2:
			continue
		if i==3 and j==1:
			continue

		neighbors = valid_neighbors(i,j)
		for a in range(nA):
			if a in neighbors:
				P[neighbors[a][0]*4+neighbors[a][1], i*4+j, a] = 1-slip_prob
				for b in neighbors:
					if b != a:
						P[neighbors[b][0]*4+neighbors[b][1], i*4+j, a] = slip_prob/float(len(neighbors.items())-1)

R = np.zeros((nS, nS, nA))

R[2,1,3] = 2000
R[2,3,2] = 2000
R[2,6,0] = 2000

R[13,9,1] = 2
R[13,14,2] = 2
R[13,12,3] = 2

R[11,15,0] = -100
R[11,7,1] = -100
R[11,10,3] = -100
R[10,14,0] = -100
R[10,6,1] = -100
R[10,11,2] = -100
R[10,9,3] = -100
R[9,10,2] = -100
R[9,13,0] = -100
R[9,5,1] = -100
R[9,8,3] = -100

env=gym.make('matrix_mdp/MatrixMDP-v0', p_0=p_0, p=P, r=R)


#################################################################
# Helper Functions
#################################################################

#reverse map observations in 0-15 to (i,j)
def reverse_map(observation):
	return observation//4, observation%4

#################################################################
# Q-Learning
#################################################################

'''
In this section, you will implement Q-learning with epsilon-greedy exploration.
The Q-table is initialized to all zeros. The value of eta should be updated as 1/(1 + number of updates) inside the loop.
The value of epsilon should be decayed to (0.9999 * epsilon) each time inside the loop.

Refer to the written assignment for the update equation. Similar to MDPs, use the following code to take an action:

observation, reward, terminated, truncated, info = env.step(action)

Unlike MDPs, your action is now chosen by the epsilon-greedy policy. The action is chosen as follows:

With probability epsilon, choose a random action.
With probability (1 - epsilon), choose the action that maximizes the Q-value (based on the last estimate). 
In case of ties, choose the action with the smallest index.
In case the chosen action is not a legal move, generate a random legal action.

The episode terminates when the agent reaches one of many terminal states. 

After 10, 100, 1000 and 10000 updates, plot a heatmap of V_opt(s) for all states s. This should be a 4x4 grid, corresponding to our map of Mordor.
Please use plt.savefig() to save the plot, and do not use plt.show().
Add each heatmap (clearly labeled) to your answer to Q9 in the written submission.
'''

Q = np.zeros((nS, nA))

gamma = 0.9
epsilon = 0.9
eta = 1

#number of episodes
num_episodes = 10000

#lists for each number of updates
V10 = []
V100 = []
V1000 = []
V10000 = []

#heatmaps
#different updates
updates = [10, 100, 1000, 10000]
heatmaps = []

#initializes V_opt
V_opt = 0

for i in tqdm(range(10001)):
	'''
	YOUR CODE HERE
	'''
	#resets the environment
	observation, info = env.reset()
	#initializes eta according to the formula
	eta = 1/(1+i)

	#initializes terminated to false
	terminated = False

	#while the terminated variable from the chosen action is true
	while not terminated:
		# checks which action to take
		if np.random.rand() < epsilon:
			action = np.random.choice(nA)	
		else:
			action = np.argmax(Q[observation])
			
		# To check if chosen action is valid for current state
		if P[:, observation, action].sum() > 0:
            # Take the action to get the next state, reward, terminated, truncated, and info from it
			next, reward, terminated, truncated, info = env.step(action)
			#fixed equation to match 
			Q[observation, action] = (1 - eta) * Q[observation, action] + eta * (reward + gamma * np.max(Q[next]))

			#change current state to next
			observation = next

			#check terminated possible continuous while loop
			if terminated:
				break

		#decays the episilon value
		epsilon *= 0.9999
		#calculates the optimal value for all states
		V_opt = np.max(Q)
		#updates the steps based on the num of updates
		if i == 10 :
			V10.append(int(V_opt))										
		elif i == 100 :
			V100.append(int(V_opt))
		elif i == 1000:
			V1000.append(int(V_opt))
		elif i == 10000:
			V10000.append(int(V_opt))

# max array length value
max = max(len(V10), len(V100), len(V1000), len(V10000))

#iterates through the number of updates
for update in updates:
	#after 10 updates
	if update == 10:
		extra = max - len(V10)
		#Some of the values in the map were invalid because the process would 
		#terminate before appending to the map so I replaced those blank spaces
		#with 0's so my map would print
		if(extra > 0):
			for i in range(extra):
				V10.append(0)
		#appended values to heatmap
		heatmaps.append(V10)
	#after 100 updates
	elif update == 100:
		extra = max - len(V100)
		#Some of the values in the map were invalid because the process would 
		#terminate before appending to the map so I replaced those blank spaces
		#with 0's so my map would print
		if(extra > 0):
			for i in range(extra):
				V100.append(0)
		#appended values to heatmap
		heatmaps.append(V100)
	#after 1000 updates
	elif update == 1000:
		extra = max - len(V1000)
		#Some of the values in the map were invalid because the process would 
		#terminate before appending to the map so I replaced those blank spaces
		#with 0's so my map would print
		if(extra > 0):
			for i in range(extra):
				V1000.append(0)
		#appended values to heatmap
		heatmaps.append(V1000)
	#after 10000 updates
	elif update == 10000:
		extra = max - len(V10000)
		#Some of the values in the map were invalid because the process would 
		#terminate before appending to the map so I replaced those blank spaces
		#with 0's so my map would print
		if(extra > 0):
			for i in range(extra):
				V10000.append(0)
		#appended values to heatmap
		heatmaps.append(V10000)

#save Q table
np.savetxt('qtable.txt', Q, fmt='%f')

policy = np.array([actions[i] for i in np.argmax(Q, axis=1)])
print(policy.reshape(4,4))


#plots the values of S1 over time for 10
plt.plot(heatmaps)
#labels for plot
plt.xlabel("Optimal S1 Values over num updates")
plt.ylabel("Number of Updates")
plt.title(f'Heatmap')
plt.imshow(heatmaps, cmap='hot')
plt.colorbar()
plt.savefig(f'heatmap_{updates}.png')
# plt.show()